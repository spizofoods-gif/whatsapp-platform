...
export default function ContactsPage() {
  ...
  const openChat = async (contact: any) => {
    ...
    try {
      const r = await fetch(`/api/messages?contact_id=${contact.id}&page=1&limit=200`);
  ...
  const handleChatSend = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        notify(e.error || 'Failed to send', 'error');
      }
    ...
    <div>
      ...
      <div style={{ display:'flex', gap:12, marginBottom:16 }}>
        <div className="stat-card" style={{ flex:1, padding:14 }}>
          <div style={{ fontSize:22,fontWeight:700,color:'#128C7E' }}>{total}</div>
          <div style={{ fontSize:12,color:'#64748b' }}>Total Contacts</div>
        </div>
        <div className="stat-card" style={{ flex:1, padding:14 }}>
          <div style={{ fontSize:22,fontWeight:700,color:'#2563eb' }}>{contacts.filter(c=>c.opted_in).length}</div>
          <div style={{ fontSize:12,color:'#64748b' }}>Opted In</div>
        </div>
      </div>
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="table-container">
          <table>
            <thead><tr><th>Name</th><th>Phone</th><th>Email</th><th>Tags</th><th>Status</th><th>Source</th><th>Added</th><th>Actions</th></tr></thead>
            <tbody>
              {loading ? <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40 }}><div className="spinner" style={{ margin: '0 auto' }} /></td></tr> :
                contacts.length === 0 ? <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: '#64748b' }}>No contacts found. Add one or import CSV.</td></tr> :
                contacts.map((c: any) => (
                  <tr key={c.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button className="btn btn-sm" style={{ background: '#25D366', color: '#fff' }} onClick={() => openChat(c)} title="Start Chat">💬 Chat</button>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleAdd}>
              <div className="modal-body">
                ...
                <div className="mb-4"><label className="label">Email</label><input className="input" type="email" placeholder="john@example.com" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></div>
      ...
      {showImport && (
        <div className="modal-overlay" onClick={() => setShowImport(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p className="text-sm text-muted mb-4">Paste CSV data. First row = headers. phone_number is required.</p>
              <textarea className="input" rows={10} style={{ fontFamily: 'monospace', fontSize: 12 }} placeholder={`phone_number,name,email\n919876543210,John Doe,john@example.com\n919876543211,Jane Doe,jane@example.com`} value={csvData} onChange={e => setCsvData(e.target.value)} />
            </div>
            <div className="modal-footer">
      ...
      {chatContact && (
        <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 420, background: '#fff', zIndex: 150, boxShadow: '-4px 0 24px rgba(0,0,0,0.12)', display: 'flex', flexDirection: 'column', animation: 'slideIn 0.25s ease' }}>
          {/* Chat Header */}
          <div style={{ padding: '14px 16px', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 10, background: '#f0fdf4' }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'linear-gradient(135deg, #25D366, #128C7E)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 16, flexShrink: 0 }}>
              {(chatContact.name || '+')[0].toUpperCase()}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 600, fontSize: 14 }}>{chatContact.name || 'Customer'}</div>
              <div style={{ fontSize: 11, color: '#64748b' }}>+{chatContact.phone_number}</div>
            </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px', background: '#efeae2', display: 'flex', flexDirection: 'column', gap: 6 }}>
            {chatMessages.length === 0 ? (
              <div style={{ display: 'flex', flex: 1, alignItems: 'center', justifyContent: 'center', textAlign: 'center', color: '#94a3b8' }}>
                <div>
                  <div style={{ fontSize: 36, marginBottom: 8 }}>💬</div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>Start a conversation</div>
                  <div style={{ fontSize: 11, marginTop: 4 }}>Send the first message below</div>
                </div>
              </div>
            ) : (
              [...chatMessages].reverse().map((m: any) => (
                <div key={m.id} style={{ display: 'flex', justifyContent: m.direction === 'outgoing' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    ...
                    background: m.direction === 'outgoing' ? '#d9fdd3' : '#fff',
                    boxShadow: '0 1px 2px rgba(0,0,0,0.06)',
                  }}>
                    <div style={{ fontSize: 13, lineHeight: 1.4, wordBreak: 'break-word' }}>{m.content || '[Media]'}</div>
                    <div style={{ fontSize: 9, color: '#667781', textAlign: 'right', marginTop: 2 }}>
                      {fmtTime(m.created_at)}
                      {m.direction === 'outgoing' ? <span style={{ marginLeft: 3 }}>{m.status === 'read' ? '✓✓' : m.status === 'delivered' ? '✓✓' : '✓'}</span> : ''}
                    </div>
                  </div>
                </div>
              ))
          ...
          <div style={{ padding: '10px 14px', background: '#f0f2f5', display: 'flex', gap: 6, alignItems: 'center', position: 'relative' }}>
            <button onClick={() => setShowEmoji(!showEmoji)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, padding: 2 }} title="Emoji">😊</button>
            <input
              ref={chatInputRef}
              style={{ flex: 1, padding: '9px 14px', border: '1px solid #fff', borderRadius: 22, fontSize: 13, outline: 'none', background: '#fff' }}
            ...
            />
            {chatReply.trim() && (
              <button onClick={handleChatSend} disabled={chatSending}
                style={{ width: 38, height: 38, borderRadius: '50%', border: 'none', background: chatSending ? '#a7f3d0' : '#25D366', color: '#fff', cursor: chatSending ? 'not-allowed' : 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {chatSending ? '⏳' : '➤'}
              </button>
            )}
            ...
            {showEmoji && (
              <div style={{ position: 'absolute', bottom: 60, left: 14, background: '#fff', borderRadius: 12, padding: 10, boxShadow: '0 8px 32px rgba(0,0,0,.15)', zIndex: 200, display: 'grid', gridTemplateColumns: 'repeat(10,1fr)', gap: 4, width: 340 }}>
                {EMOJIS.map(e => (
                  <button key={e} onClick={() => { setChatReply(chatReply + e); setShowEmoji(false); chatInputRef.current?.focus(); }}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 18, padding: 4, borderRadius: 6 }}
                    onMouseEnter={e2 => e2.currentTarget.style.background = '#f1f5f9'}
