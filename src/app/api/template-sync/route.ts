    ...
    for (const tpl of templates) {
      try {
        ...
        try {
          const id = uuidv4();
          await sql`
            DELETE FROM templates WHERE whatsapp_template_id = ${tpl.name}
          `;
          await sql`
            INSERT INTO templates (id, name, category, language, header_type, body, status, whatsapp_template_id, created_at, updated_at)
            VALUES (${id}, ${tpl.name}, ${(tpl.category || 'marketing').toLowerCase()}, ${tpl.language || 'en'},
              'none', ${body?.text || ''}, ${tpl.status || 'PENDING'}, ${tpl.name}, NOW(), NOW())
          `;
          synced.push({ name: tpl.name, status: tpl.status, header: 'none', buttons: 0 });
        } catch (e2: any) {
          console.error('Fallback save also failed:', e2.message);
        }
      }
    }
