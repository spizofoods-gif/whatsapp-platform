  ...
  try {
    ...
    if (templates.length === 0) {
      ...
      if (altId && altId !== accountId) {
        ...
        return NextResponse.json({ success: true, templates: t2.map((t: any) => formatTemplate(t)), synced: t2.length, account: altId });
      }
      return NextResponse.json({ templates: [], synced: 0, account: accountId });
    }

    return NextResponse.json({ success: true, templates: templates.map((t: any) => formatTemplate(t)), synced: templates.length, account: accountId });
  } catch (err: any) {
