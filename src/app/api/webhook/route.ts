...
import { getAllSettings, findOrCreateContact, saveMessage, getChatbotRules, getCampaignRecipientByMessageId, updateCampaignRecipient, updateCampaign, getCampaign, logWebhook } from '@/lib/db';
  ...
  try {
    ...
    if (msg) {
      ...
      if (settings['auto_reply_enabled'] === 'true') {
        ...
        if (!matched && settings['auto_reply_message']) {
          await sendTextMessage(phone, settings['auto_reply_message']).catch(() => {});
        }
      }
    }
    ...
    if (status) {
      ...
      const cr = await getCampaignRecipientByMessageId(status.id);
      if (cr) {
        await updateCampaignRecipient(cr.id, {
          status: ns,
          delivered_at: ns === 'delivered' ? new Date().toISOString() : cr.delivered_at,
          read_at: ns === 'read' ? new Date().toISOString() : cr.read_at,
        });
        const camp = await getCampaign(cr.campaign_id) as any;
        if (camp) {
          if (ns === 'delivered') await updateCampaign(cr.campaign_id, { delivered_count: (camp.delivered_count || 0) + 1 });
          if (ns === 'read') await updateCampaign(cr.campaign_id, { read_count: (camp.read_count || 0) + 1 });
        }
      }
    }
